package com.example.projectIot.beans;
...
public class MqttBeans {
    ...
    public MqttPahoClientFactory mqttClientFactory(){
        ...
        options.setServerURIs(new String[] { "ssl://3dbd38b67df14bf9ab01f38fa97561ab.s2.eu.hivemq.cloud:8883" });
            ...
            public void handleMessage(Message<?> message) throws MessagingException {
                ...
                if(topic.equals("remote")) {
                    ...
                    try {
                        ...
                        // ObjectMapper được sử dụng để thực hiện các hoạt động liên quan đến JSON,
                        // bao gồm việc phân tích và chuyển đổi dữ liệu JSON.
                        ObjectMapper objectMapper = new ObjectMapper();

                        JsonNode jsonNode = objectMapper.readTree(payload);

                        double temp = jsonNode.get("temp").asDouble(); // Phương thức này được sử dụng để chuyển đổi giá trị từ dạng JSON thành kiểu dữ liệu double.
                        double humidity = jsonNode.get("humidity").asDouble();
                        double light = jsonNode.get("brightness").asDouble();

//                        double dust = jsonNode.get("dust").asDouble();
...
//                        data.setDust(dust);
                        ...
                        dataRepo.save(data);
                        simpMessagingTemplate.convertAndSend("/topic/message", data);

//                        System.out.println(temp + " " + humidity + " " + light + " " + formattedDateTime);
                    } catch(Exception e) {
                        e.printStackTrace();
                    }
                }
...
// int randomNumber = random.nextInt(100); // Số nguyên ngẫu nhiên từ 0 đến 99